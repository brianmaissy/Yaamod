...
class MembersTestCase(TestCase):
    def setUp(self):
        ...
        self.father = Person.objects.create(
            formal_name='Dad', synagogue=self.synagogue)
        self.mother = Person.objects.create(
            formal_name='Mom', synagogue=self.synagogue)
        self.daughter = Member.objects.create(
            formal_name='Sarah', synagogue=self.synagogue, last_name='Levi',
            date_of_birth=date(2019, 1, 1), father=self.father,
            mother=self.mother)
        self.son = MaleMember.objects.create(
            formal_name='Reuven', synagogue=self.synagogue, last_name='Levi',
            date_of_birth=today, father=self.father, mother=self.mother,
            yichus=Yichus.LEVI)
        self.wife = Member.objects.create(
            formal_name='Rivkah', synagogue=self.synagogue, last_name='Shtern',
            date_of_birth=today,
            father=Person.objects.create(formal_name='Father in Law',
                                         synagogue=self.synagogue),
            mother=Person.objects.create(formal_name='Mother in Law',
                                         synagogue=self.synagogue))
...
class TestSynagogue(MembersTestCase):
    def test_members(self):
        ...
        self.assertEquals(set(self.synagogue.member_set.all()),
                          {self.daughter, self.son.member_ptr, self.wife})
...
class TestPerson(MembersTestCase):
    def test_sanity(self):
        ...
        self.daughter.save()
        self.assertEquals(self.father.hebrew_date_of_death,
...
class TestMember(MembersTestCase):
    def test_sanity(self):
        ...
        self.assertEquals(self.daughter.hebrew_date_of_birth,
                          HebrewDate(5779, 10, 24))
...
class TestMaleMember(MembersTestCase):
    ...
    def test_can_get_aliya(self):
        self.assertEquals(self.son.bar_mitzvah_date,
                          nth_anniversary_of(self.son.hebrew_date_of_birth, 13))
        self.assertFalse(self.son.is_bar_mitzvah)
        self.assertFalse(self.son.can_get_aliya)

        self.son.date_of_birth = self.son.date_of_birth.replace(
            year=self.son.date_of_birth.year - 14)
        self.son.save()
